...
timepoint5_dataset = pd.DataFrame(dataset.iloc[:,dataset.columns.get_loc("x__5"):dataset.columns.get_loc("x__6")])
...
active_cells = []
for i in list(timepoint5_dataset["ftz__5"]):
    if i > ftz_threshold:
        active_cells += [1]
    else:
        active_cells += [0]
...
threshold_fig.add_trace(
        go.Scatter3d(
            x=timepoint5_dataset["x__5"],
            y=timepoint5_dataset["y__5"],
            z=timepoint5_dataset["z__5"],
            mode='markers',
...
for i in (np.array(range(7))+1): #numbers of stripes
    stripes[str(i)] = [0] * len(timepoint5_dataset["ftz__5"])
...
while min(abs(np.array(list(z.values())))) < 160:
    ...
    for i in stripes.keys():
        ...
        for j in range(len(timepoint5_dataset["ftz__5"])):
            if ((timepoint5_dataset["x__5"][j] > (x[str(i)])) & (timepoint5_dataset["x__5"][j] < (x[str(i)] + plus_x[str(i)])) & \
                (timepoint5_dataset["z__5"][j] > (z[str(i)]-2)) & (timepoint5_dataset["z__5"][j] < (z[str(i)] + 2)) & (timepoint5_dataset["ftz__5"][j] > ftz_threshold)):
                stripes[str(i)][j] = 1
            else:
                pass

#making a single dolumn specifing cell stripe identity
all_stripes = [0] * len(timepoint5_dataset["ftz__5"])
for i in stripes.keys():
    for j in range(len(timepoint5_dataset["ftz__5"])):
...
stripes_export.to_csv("ftz\Jans_ftz_stripes_from_bdtnp.csv", index=False)
