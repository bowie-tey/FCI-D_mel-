...
for z,j,i in zip(gene,gene_3,range(1, 44)):
    ax = fig.add_subplot(7, 7, i, projection="rectilinear")
    ax.set_xlim([-220, 220])  # -220,+220
    ax.set_ylim([-220, 220])
    #ax.set_xlabel('Anteroposterior Axis',size =3)
    #ax.set_ylabel('Dorsoventral Axis',size = 3)
    ax.title.set_text(f'{z}')  # without it they are bigger
    #ax.set_zlim([-220, 220])
    ax.scatter(gene_3['x__3'], gene_3['z__3'], c= gene_3[j], s=7, marker='.',
               cmap='Blues')  # s = 30
    ax.set_yticklabels([])
    ax.set_xticklabels([])
